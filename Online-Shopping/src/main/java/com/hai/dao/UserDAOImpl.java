...
public class UserDAOImpl implements IUserDAO {
	...
	public List<Users> findByProperty(String name, Object proValue) {
		...
		// Check proValue
		if (proValue.equals("") || proValue == null) {
			return null;
		}
		// Open session
		session = sessionFactory.openSession();
		...
		try {
			...
			if (query.getResultList() != null)
				users = query.getResultList();
			session.getTransaction().commit();
			LOGGER.debug("Fetch USer sucessfully");
			return users;
		} catch (Exception e) {
	...
	public Users findByEmail(String email) {
		...
		LOGGER.debug("Call find Users by email");
		...
		try {
			...
			Root<Users> categoryRoot = criteriaQuery.from(Users.class);
			ParameterExpression<Object> paramValue = builder.parameter(Object.class);
			criteriaQuery.select(categoryRoot).where(builder.equal(categoryRoot.get("email"), paramValue));
			Query<Users> query = session.createQuery(criteriaQuery);
